...
type Snapshot struct {
	...
	// HotStuff tracking fields
	CurrentView uint64      `json:"current_view"`
	HighQCView  uint64      `json:"high_qc_view"`
	HighQCHash  common.Hash `json:"high_qc_hash"`
	HighTCView  uint64      `json:"high_tc_view"`
}
...
func (s *Snapshot) copy() *Snapshot {
	cpy := &Snapshot{
		...
		HighQCView:       s.HighQCView,
		HighQCHash:       s.HighQCHash,
		HighTCView:       s.HighTCView,
		Validators:       make(map[common.Address]*ValidatorInfo),
		Recents:          make(map[uint64]common.Address),
		RecentForkHashes: make(map[uint64]string),
	}
...
func (s *Snapshot) apply(headers []*types.Header, chain consensus.ChainHeaderReader, parents []*types.Header, chainConfig *params.ChainConfig, getBlockFromState func(common.Hash) *types.Block) (*Snapshot, error) {
	...
	for _, header := range headers {
		...
		if hasSyncInfo {
			...
			if snap.HighQCView < hqcView {
				log.Debug("updateHighQCView on snapshot", "number", number, "oldView", snap.HighQCView, "newView", hqcView)
				snap.HighQCView = hqcView
				snap.HighQCHash = hqcHash
			}
			// Update HighTC independently if view increased (TC and QC can advance independently)
			if snap.HighTCView < htcView {
				log.Debug("updateHighTCView on snapshot", "number", number, "oldView", snap.HighTCView, "newView", htcView)
				snap.HighTCView = htcView
			}
		}
		...
		if isEpochBoundary {
			...
			log.Debug("parseValidators", "checkpointHeader", checkpointHeader.Number.Uint64(), "epochLength", epochLength)
			newValArr, voteAddrs, err := parseValidators(checkpointHeader, chainConfig, epochLength)
			if err != nil {
				return nil, err
			}
			log.Debug("newValArr", "len", len(newValArr))
			for _, val := range newValArr {
				log.Debug("val", "val", val.String())
			}
			for _, voteAddr := range voteAddrs {
				log.Debug("voteAddr", "voteAddr", hex.EncodeToString(voteAddr[:]))
			}
...
// inturnValidatorAt calculates the inturn validator for a specific block number
// This is useful when using a snapshot from a different block (e.g., grandparent)
func (s *Snapshot) inturnValidatorAt(blockNumber uint64) common.Address {
	validators := s.validators()
	offset := (blockNumber + 1) / uint64(s.TurnLength) % uint64(len(validators))
	return validators[offset]
}
...
func parseTurnLength(header *types.Header, chainConfig *params.ChainConfig, epochLength uint64) (*uint8, error) {
	...
	// Ensure we don't read into syncInfo or extraSeal
	maxPos := len(header.Extra) - extraSeal - hsExtraSize
	if pos >= maxPos {
...
// FindAncientHeader finds an ancestor header by traversing back `ite` blocks.
// It searches in the following order:
// 1. candidateParents slice (for pipelined blocks)
// 2. Canonical chain via chain.GetHeader()
// 3. HotStuff state via getBlockFromState (for uncommitted blocks)
func FindAncientHeader(header *types.Header, ite uint64, chain consensus.ChainHeaderReader, candidateParents []*types.Header, getBlockFromState func(common.Hash) *types.Block) *types.Header {
	ancient := header
	for i := uint64(1); i <= ite; i++ {
		...
		// Try 3: HotStuff state (for pipelined uncommitted blocks)
		if !found && getBlockFromState != nil {
			if block := getBlockFromState(parentHash); block != nil {
				ancient = block.Header()
				found = true
				log.Debug("FindAncientHeader: got header from HotStuff state",
		...
		if ancient == nil || !found {
			log.Warn("FindAncientHeader: parent not found",
				"parentHash", parentHash.Hex()[:10],
				"parentHeight", parentHeight,
				"iteration", i,
				"totalIterations", ite)
