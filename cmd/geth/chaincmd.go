...
var (
	initCommand = &cli.Command{
		...
		Flags: slices.Concat([]cli.Flag{
			...
			utils.OverrideCopperRemix2Fix,
			utils.OverrideCopperRemix2FixRewardJailed,
		}, utils.DatabaseFlags),
		Description: `
...
func initGenesis(ctx *cli.Context) error {
	...
	if ctx.IsSet(utils.OverrideCopperRemix.Name) {
		v := ctx.Uint64(utils.OverrideCopperRemix.Name)
		overrides.OverrideCopperRemix = &v
	}
	if ctx.IsSet(utils.OverrideCopperRemixFix.Name) {
		v := ctx.Uint64(utils.OverrideCopperRemixFix.Name)
		overrides.OverrideCopperRemixFix = &v
	}
	if ctx.IsSet(utils.OverrideCopperRemix2.Name) {
		v := ctx.Uint64(utils.OverrideCopperRemix2.Name)
		overrides.OverrideCopperRemix2 = &v
	}
	if ctx.IsSet(utils.OverrideCopperRemix2Fix.Name) {
		v := ctx.Uint64(utils.OverrideCopperRemix2Fix.Name)
		overrides.OverrideCopperRemix2Fix = &v
	}
	if ctx.IsSet(utils.OverrideCopperRemix2FixRewardJailed.Name) {
		v := ctx.Uint64(utils.OverrideCopperRemix2FixRewardJailed.Name)
		overrides.OverrideCopperRemix2FixRewardJailed = &v
	}
